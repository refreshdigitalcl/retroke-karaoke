  ...
  try {
    image = new ImageResponse(
      (
        ...
        >
          {/* HERO: caratula del album a pantalla completa, con el logo y
              el nombre del cantante superpuestos (ver ShareResultCard.jsx
              para el diseño equivalente que se ve en vivo en la pagina). */}
          <div style={{ position: 'relative', display: 'flex', width: '100%', height: 900, backgroundColor: '#150a20' }}>
            {artworkDataUri ? (
              <img
                src={artworkDataUri}
                width={WIDTH}
            ...
            <div
              style={{
                ...
                display: 'flex',
                background:
                  'linear-gradient(180deg, rgba(10,5,18,0.62) 0%, rgba(10,5,18,0.08) 28%, rgba(10,5,18,0.16) 58%, rgba(10,5,18,0.88) 100%)'
              }}
            />
            <div style={{ position: 'absolute', top: 55, left: 0, width: '100%', display: 'flex', justifyContent: 'center' }}>
              {logoDataUri && <img src={logoDataUri} width={222} height={82} style={{ objectFit: 'contain' }} />}
            </div>
            <div style={{ position: 'absolute', left: 60, right: 60, bottom: 55, display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', fontSize: 76, fontWeight: 800, color: '#fff' }}>{perf.singer_name || 'Cantante Retroke'}</div>
              {levelName && (
                <div
                  style={{
                    display: 'flex',
          ...
          {/* FICHA: cancion + resultados + lugar/fecha + marca */}
          <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', flex: 1, padding: '55px 70px', backgroundColor: '#0a0512' }}>
            <div style={{ display: 'flex', justifyContent: 'center', fontSize: 46, fontWeight: 800, color: '#fff', textAlign: 'center' }}>
              {perf.song || 'Canción'}
            </div>
            {perf.artist_name && (
              <div style={{ display: 'flex', justifyContent: 'center', marginTop: 10, fontSize: 34, fontWeight: 600, color: 'rgba(255,255,255,0.68)' }}>
                {perf.artist_name}
              </div>
            )}
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: hasReactions || hasVocalScore ? 1.15 : 1 }}>
                <div style={{ display: 'flex', fontSize: 26, letterSpacing: 3, color: 'rgba(255,255,255,0.55)' }}>⭐ NOTA</div>
                <div style={{ display: 'flex', marginTop: 8, fontSize: 108, fontWeight: 700, color: '#F4D03F' }}>{notaTxt}</div>
              </div>
              {hasReactions && (
                <>
                  <div style={{ display: 'flex', width: 3, backgroundColor: 'rgba(255,255,255,0.16)' }} />
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                    <div style={{ display: 'flex', fontSize: 26, letterSpacing: 3, color: 'rgba(255,255,255,0.55)' }}>🔥 REACCIONES</div>
                    <div style={{ display: 'flex', marginTop: 14, fontSize: 58, letterSpacing: 8 }}>
              ...
              {hasVocalScore && !hasReactions && (
                <>
                  <div style={{ display: 'flex', width: 3, backgroundColor: 'rgba(255,255,255,0.16)' }} />
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                    <div style={{ display: 'flex', fontSize: 24, letterSpacing: 2, color: 'rgba(255,255,255,0.55)' }}>🎤 RETROKE SCORE</div>
                    <div style={{ display: 'flex', marginTop: 8, fontSize: 54, fontWeight: 700, color: '#E91E8C' }}>{perf.vocal_score}/100</div>
                  </div>
                </>
              )}
            </div>
            ...
            {modeMeta && (
              ...
              >
                ...
                {placeName && (
                  <>
                    <div style={{ display: 'flex', width: 8, height: 8, borderRadius: 999, backgroundColor: 'rgba(255,255,255,0.35)' }} />
                    <div style={{ display: 'flex', fontSize: 32, color: 'rgba(255,255,255,0.55)' }}>{placeName}</div>
                  </>
                )}
                {dateTxt && (
                  <>
                    <div style={{ display: 'flex', width: 8, height: 8, borderRadius: 999, backgroundColor: 'rgba(255,255,255,0.35)' }} />
                    <div style={{ display: 'flex', fontSize: 32, color: 'rgba(255,255,255,0.55)' }}>{dateTxt}</div>
                  </>
                )}
              </div>
            )}

            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 'auto', paddingTop: 55 }}>
              <div style={{ display: 'flex', fontSize: 34, fontWeight: 600, color: 'rgba(255,255,255,0.75)' }}>El karaoke cambió para siempre.</div>
              <div style={{ display: 'flex', marginTop: 6, fontSize: 28, color: 'rgba(255,255,255,0.42)' }}>retroke.cl</div>
            </div>
          </div>
        </div>
      ),
