...
export default function DisplayQueue() {
        ...
        const barName = session.barName
        const sessionCode = session.sessionCode
        const queue = session.queue
        const currentSinger = session.currentSinger
            ...
            <div className="min-h-screen relative overflow-hidden px-8 py-10 flex flex-col bg-black">
                  ...
                  <section className="relative z-10 text-center mb-10">
                        ...
                        {currentSinger && (
                            <div className="inline-flex items-center gap-5 rounded-2xl px-9 py-5 border-2 border-pink-600 bg-neutral-900">
                                        ...
                                        <div className="text-left">
                                                      <p className="text-2xl font-medium text-white">{currentSinger.name}</p>p>
                                                      <p className="text-base mt-0.5 text-purple-400">{currentSinger.song}</p>p>
                                        </div>div>
                            </div>div>
                          )}
                        ...
                        {!currentSinger && (
                            <div className="inline-block rounded-2xl px-9 py-6 border border-neutral-700 bg-neutral-900">
                                        <p className="text-neutral-400">Esperando al próximo cantante</p>p>
                            </div>div>
                          )}
                  </section>section>
                  ...
                  <section className="relative z-10 max-w-lg w-full mx-auto flex-1">
                          ...
                          <div className="flex flex-col gap-2.5">
                                {queue.length === 0 && (
                              <p className="text-center text-sm text-neutral-500">La cola está vacía. Escanea el QR para anotarte.</p>p>
                                    )}
